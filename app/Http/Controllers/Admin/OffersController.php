...
class OffersController extends Controller
{
    ...
    {
        $this->middleware('permission:offers_list')->only(['index', 'getData']);
        $this->middleware('permission:offers_create')->only(['create', 'store']);
        $this->middleware('permission:offers_edit')->only(['edit', 'update', 'changeStatus', 'bulkChangeStatus']);
        $this->middleware('permission:offers_delete')->only(['destroy', 'bulkDelete']);
    }
    ...
    public function bulkDelete(Request $request)
    {
        try {
            ...
            $validator = Validator::make(['ids' => $ids], [
                'ids'   => 'required|array',
                'ids.*' => 'required|integer|exists:offers,id',
            ]);
            if (! is_array($validator) && $validator->fails()) {
                return redirect()->back()->withErrors($validator->validated());
            }
            ...
            ->addColumn('action', function ($row) {
                $actionButtons = '
                    <li class="list-inline-item edit" data-bs-toggle="tooltip" data-bs-trigger="hover" data-bs-placement="top" title="' . __('Edit') . '">
                        <a href="' . route('offers.edit', $row->id) . '" class="text-primary d-inline-block edit-item-btn">
    ...
    public function changeStatus(Request $request)
    {
        ...
        $category            = Offer::findOrFail($request->id);
