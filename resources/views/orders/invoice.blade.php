                        ...
                        <div class="card-header border-bottom-dashed p-4">
                            <div class="d-flex">
                                <div class="flex-grow-1">
                                    <img src="{{ URL::asset('build/images/logo.png') }}" class="card-logo card-logo-dark" alt="@lang('Fann')" height="50">
{{--                                    <h1 style="font-weight: bolder">@lang('Fann')</h1>--}}
                                    <div class="mt-sm-5 mt-4">
                                        <h6 class="text-muted text-uppercase fw-semibold">@lang('Address')</h6>
                                        <p class="text-muted mb-1" id="address-details">{{ optional($order->address)->address }}</p>
                                    </div>
                                </div>
                                <div class="flex-shrink-0 mt-sm-0 mt-3">
                                    <h6><span class="text-muted fw-normal">@lang('registration_number'):</span>
                                        <span id="legal-register-no">{{ $settings['registration_number'] }}</span>
                                    </h6>
                                    <h6><span class="text-muted fw-normal">@lang('Email'):</span>
                                        <span id="email">{{ $settings['email'] }}</span>
                                    </h6>
                                    <h6><span class="text-muted fw-normal">@lang('website'):</span> <a href="{{ $settings['website'] }}" class="link-primary" target="_blank" id="website">{{ $settings['website'] }}</a></h6>
                                    <h6 class="mb-0"><span class="text-muted fw-normal">@lang('Contact No'): {{ $settings['phone'] }}</span><span id="contact-no">  </span></h6>
                                </div>
                            </div>
                        </div>
                        <!--end card-header-->
                        ...
                        <div class="card-body p-4">
                            <div class="row g-3">
                                <div class="col-lg-4 col-4">
                                    <p class="text-muted mb-2 text-uppercase fw-semibold">@lang('Invoice No')</p>
                                    <h5 class="fs-14 mb-0"><span id="invoice-no">{{ $order->order_number }}</span></h5>
                                </div>
                                <!--end col-->
                                <div class="col-lg-4 col-4">
                                    <p class="text-muted mb-2 text-uppercase fw-semibold">@lang('Order Date')</p>
                                    <h5 class="fs-14 mb-0"><span id="invoice-date">{{ \Carbon\Carbon::parse($order->created_at)->toDateString() }}</span> </h5>
                                </div>
                                <div class="col-lg-4 col-4">
                                    <p class="text-muted mb-2 text-uppercase fw-semibold">@lang('Order Time')</p>
                                    <h5 class="fs-14 mb-0"><span id="invoice-date">{{ localizedTime($order->created_at) }}</span> </h5>
                                </div>
                                <!--end col-->
                                <div class="col-lg-4 col-4">
                                    <p class="text-muted mb-2 text-uppercase fw-semibold">@lang('Order Status')</p>
                                    @php
                                        if ($order->status == \App\Enums\OrderStatusEnum::PENDING->value) {
                                            $class = "bg-warning-subtle text-warning";
                                        }elseif ($order->status == \App\Enums\OrderStatusEnum::IN_PROGRESS->value) {
                                            $class = "bg-primary-subtle text-primary";
                                        }elseif ($order->status == \App\Enums\OrderStatusEnum::COMPLETE->value) {
                                            $class = "bg-success-subtle text-success";
                                        }else {
                                            $class = "bg-danger-subtle text-danger";
                                        }
                                    @endphp
                                    <span class="badge {{$class}} fs-11" id="payment-status">{{ __($order->status) }}</span>
                                </div>
                                <div class="col-lg-4 col-4">
                                    <p class="text-muted mb-2 text-uppercase fw-semibold">@lang('Payment Method')</p>
                                    <h5 class="fs-14 mb-0"><span id="invoice-date">{{ __($order->payment_method) }}</span> </h5>
                                </div>
                                <!--end col-->
                                <div class="col-lg-4 col-4">
                                    <p class="text-muted mb-2 text-uppercase fw-semibold">@lang('Total Amount')</p>
                                    <h5 class="fs-14 mb-0">
                                ...
                                <table class="table table-borderless text-center table-nowrap align-middle mb-0">
                                    ...
                                    <tbody id="products-list">
                                        @foreach($order->items as $item)
                                            <tr @if(! $loop->first) class="border-top border-top-double" @endif>
                                                @if($item->target_type === 'App\Models\Product')
                                                    @component('components.product-item', ['product' => $item->target, 'item' => $item])
                                                    @endcomponent
                                                @elseif($item->target_type === 'App\Models\Service')
                                                    @component('components.service-item', ['service' => $item->target, 'item' => $item])
                                                    @endcomponent
                                                @endif
                                            </tr>
                                        @endforeach
